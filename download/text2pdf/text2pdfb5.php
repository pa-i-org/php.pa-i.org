...
class	page {
	...
	function	page($title = "") {
		...
		$this->margin_top = $this->mm2pnt(297 - 13) + $this->linepitch;
		$this->settitle($title);
	}
	function	mm2pnt($val) {
		return $val / 25.4 * 72;
	...
	function	flushpage() {
		if ($this->in_page == 0)
			return;
		pdf_set_text_pos($this->pid, $this->mm2pnt(210 - 30) - strlen($this->title) * $this->fontsize / 2, $this->mm2pnt(297 - 8));
		$this->putstringh($this->title);
		pdf_set_text_pos($this->pid, ($this->mm2pnt(210) - $this->fontsize) / 2, $this->mm2pnt(6));
